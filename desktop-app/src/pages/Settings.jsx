...
const Settings = ({ user, shopSettings, onSettingsUpdate }) => {
    ...
    const [openSection, setOpenSection] = useState('account'); // accordion state
    ...
    // Session State
    const [sessionTimeoutEnabled, setSessionTimeoutEnabled] = useState(shopSettings.session_timeout_enabled === '1');
    const [sessionTimeoutMinutes, setSessionTimeoutMinutes] = useState(shopSettings.session_timeout_minutes || '10');
    ...
    const handleUpdateAccount = async (e) => {
        ...
        setSaving(true);
        try {
            const result = await window.electron.updateAccount({
                username,
                currentPassword,
                newPassword
            });
            ...
            if (result.success) {
                ...
                Swal.fire('Error', result.error || 'Failed to update account', 'error');
            }
        } catch (err) {
    ...
    const handleSaveSessionSettings = async () => {
        setSaving(true);
        try {
            await window.electron.saveSetting('session_timeout_enabled', sessionTimeoutEnabled ? '1' : '0');
            await window.electron.saveSetting('session_timeout_minutes', sessionTimeoutMinutes);
            ...
            Swal.fire('Error', 'Failed to save session settings', 'error');
        } finally {
            setSaving(false);
        }
    };
            ...
            <div className="grid grid-cols-1 gap-4">
                ...
                <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                    <button 
                        ...
                        className={`w-full p-8 lg:p-10 flex items-center justify-between transition-colors ${openSection === 'account' ? 'bg-slate-50/80' : 'hover:bg-slate-50/50'}`}
                    >
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 bg-slate-800 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-slate-100">
                            ...
                            <div className="text-left">
                                ...
                                <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest">Update credentials</p>
                            </div>
                        </div>
                        <svg className={`w-6 h-6 text-slate-400 transition-transform duration-300 ${openSection === 'account' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7" /></svg>
                    </button>

                    {openSection === 'account' && (
                        <div className="p-8 lg:p-10 border-t border-slate-50 space-y-8 animate-slide-down">
                            <form onSubmit={handleUpdateAccount} className="space-y-6">
                                ...
                                <div className="flex pt-4">
                                    <button 
                                        type="submit"
                                        disabled={saving}
                                        className="flex-1 px-8 py-4 bg-slate-900 hover:bg-slate-800 text-white font-black rounded-2xl shadow-xl shadow-slate-100 transition-all uppercase tracking-widest text-xs"
                                    >
                                        {saving ? 'Saving...' : 'Update Account Info'}
                                    </button>
                                </div>
                            </form>
                        </div>
                ...
                <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                    <button 
                        ...
                        className={`w-full p-8 lg:p-10 flex items-center justify-between transition-colors ${openSection === 'shop' ? 'bg-slate-50/80' : 'hover:bg-slate-50/50'}`}
                    >
                        <div className="flex items-center gap-4">
                        ...
                        <svg className={`w-6 h-6 text-slate-400 transition-transform duration-300 ${openSection === 'shop' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7" /></svg>
                    </button>
                ...
                <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                    <button 
                        ...
                        className={`w-full p-8 lg:p-10 flex items-center justify-between transition-colors ${openSection === 'cashier' ? 'bg-slate-50/80' : 'hover:bg-slate-50/50'}`}
                    >
                        <div className="flex items-center gap-4">
                        ...
                        <svg className={`w-6 h-6 text-slate-400 transition-transform duration-300 ${openSection === 'cashier' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7" /></svg>
                    </button>

                    {openSection === 'cashier' && (
                        <div className="p-8 lg:p-10 border-t border-slate-50 space-y-6 animate-slide-down">
                ...
                <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                    <button 
                        ...
                        className={`w-full p-8 lg:p-10 flex items-center justify-between transition-colors ${openSection === 'printer' ? 'bg-slate-50/80' : 'hover:bg-slate-50/50'}`}
                    >
                        <div className="flex items-center gap-4">
                        ...
                        <svg className={`w-6 h-6 text-slate-400 transition-transform duration-300 ${openSection === 'printer' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7" /></svg>
                    </button>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <button onClick={loadPrinters} className="text-brand-600 font-black text-[10px] uppercase tracking-widest hover:underline flex items-center gap-1">
                                            <svg className={`w-3 h-3 ${loadingPrinters ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                                            Refresh
                                        </button>
                                    </div>
                                    <select 
                ...
                <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
                    <button 
                        ...
                        className={`w-full p-8 lg:p-10 flex items-center justify-between transition-colors ${openSection === 'session' ? 'bg-slate-50/80' : 'hover:bg-slate-50/50'}`}
                    >
                        <div className="flex items-center gap-4">
                            ...
                            <div className="text-left">
                                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">Session & Security</h3>
                                <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest">Auto-logout and idle timeout</p>
                            </div>
                        </div>
                        <svg className={`w-6 h-6 text-slate-400 transition-transform duration-300 ${openSection === 'session' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7" /></svg>
                    </button>
                    ...
                    {openSection === 'session' && (
                        <div className="p-8 lg:p-10 border-t border-slate-50 space-y-8 animate-slide-down">
                            <div className="flex items-center justify-between p-6 bg-slate-50 rounded-[1.5rem] border border-slate-100">
                                <div className="space-y-1">
                                    <h4 className="font-black text-slate-900 uppercase tracking-tight text-sm">Enable Idle Timeout</h4>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Automatically log out after being idle</p>
                                </div>
                                <button 
                                    onClick={() => setSessionTimeoutEnabled(!sessionTimeoutEnabled)}
                                    className={`w-14 h-8 rounded-full transition-all relative ${sessionTimeoutEnabled ? 'bg-brand-600' : 'bg-slate-200'}`}
                                >
                                    <div className={`absolute top-1 w-6 h-6 bg-white rounded-full shadow-sm transition-all ${sessionTimeoutEnabled ? 'left-7' : 'left-1'}`}></div>
                                </button>
                            </div>

                            <div className={`space-y-3 transition-opacity duration-300 ${sessionTimeoutEnabled ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Idle Time (Minutes)</label>
                                <div className="flex items-center gap-4">
                                    <input 
                                        type="number" 
                                        min="1"
                                        ...
                                        className="flex-1 px-6 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-brand-500/10 focus:border-brand-500 transition-all outline-none font-bold text-slate-900"
                                    />
                                    <span className="text-slate-400 font-black uppercase tracking-widest text-xs">Minutes</span>
                                </div>
                            </div>
                            ...
                            <div className="pt-4 border-t border-slate-50">
                                <button 
                                    ...
                                    disabled={saving}
                                    className="w-full px-8 py-4 bg-amber-600 hover:bg-amber-700 text-white font-black rounded-2xl shadow-xl shadow-amber-100 transition-all uppercase tracking-widest text-xs"
                                >
                                    {saving ? 'Saving...' : 'Update Session Security'}
                                </button>
                            </div>
                        </div>
                    )}
