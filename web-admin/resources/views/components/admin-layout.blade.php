            ...
            <aside class="hidden lg:flex flex-col w-72 bg-white border-r border-slate-100 shrink-0">
                ...
                <nav class="flex-1 p-6 space-y-2 overflow-y-auto">
                    ...
                    @if(Auth::user()->isSuperAdmin())
                        <a href="{{ route('admin.master-licenses.index') }}" class="flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all duration-200 {{ request()->routeIs('admin.master-licenses.*') ? 'sidebar-link-active' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900 font-bold' }}">
